...
<template>
    <div class="left relative w-1/5 px-2 h-full border flex flex-col " v-show="store.isShow">
        ...
        <div class="basis-10/12 overflow-auto ">
            ...
            <ChatItem v-for="(chat, index) in store.chatStorage.data.sidebar" :key="index" :idx="chat['uuid']" v-model:content="chat['title']"
    ...
    <span @click="sidebarFlex">
        <div class="  absolute top-1/3 flex flex-col">
            ...
            <div class=" w-1/3 font-medium ltpic-main flex justify-center items-center">{{ store.isShow ? "&lt;": "&gt;" }}</div>
            <div class=" w-1/3  b-show">1 </div>

        </div>
        <!-- <span class="w-10 h-10 border absolute top-1/3 left-1/5  ddddddd">123</span> -->
